...
# Define paths to binary file for LUT creation which is one level up in bin directory
path = Path(__file__).parents[1]
binPath = str(path)+'/bin/gm_lab.bin'
...
lcmswrap.applycform.argtypes = [ctypes.c_void_p, ctypes.POINTER(
...
lcmswrap.makeabscform.argtypes = [
    ctypes.POINTER(ctypes.c_double), ctypes.c_uint]
lcmswrap.makeabscform.restype = ctypes.c_void_p
lcmswrap.applyabscform.argtypes = [ctypes.c_void_p, ctypes.POINTER(
    ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_size_t]
...
revint.makegammapForm.argtypes = [
...
revint.makelabk2cmykForm.argtypes = [
    ctypes.POINTER(ctypes.c_float), ctypes.c_size_t]
revint.makelabk2cmykForm.restype = ctypes.c_void_p
revint.applygammapForm.argtypes = [ctypes.c_void_p, ctypes.POINTER(
    ctypes.c_float), ctypes.POINTER(ctypes.c_float), ctypes.c_size_t]
revint.applygammapForm.restype = None
revint.applylab2kForm.argtypes = [ctypes.c_void_p, ctypes.POINTER(
    ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_uint]
revint.applylab2kForm.restype = None
revint.applylabk2cmykForm.argtypes = [ctypes.c_void_p, ctypes.POINTER(
    ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_size_t]
...
revint.getLabExtmRng.argtypes = [ctypes.c_void_p, ctypes.POINTER(
    ctypes.c_double), ctypes.POINTER(ctypes.c_double)]
revint.getLabExtmRng.restype = None
# Setup Gamut mapping algorithm from RevInt wrapper
revint.makegma.argtypes = [ctypes.POINTER(
    ctypes.c_double), ctypes.c_size_t, ctypes.POINTER(ctypes.c_int32), ctypes.c_size_t]
revint.makegma.restype = ctypes.c_void_p
revint.map.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_double), ctypes.POINTER(
    ctypes.c_double), ctypes.c_size_t, ctypes.POINTER(ctypes.c_uint16), PRINTPROGRESS]
...
class cform:
    ...
    def __init__(self, profpath, luttype):
        ...
        self.address = ctypes.c_void_p(
            lcmswrap.makecform(profpath_p, luttype_p))
    ...
    def apply(self, inmat):
        ...
        outmat = np.ndarray(shape=(
            1, np.int(np.size(inmat, 0)*self.no_out_ch)), dtype=ctypes.c_double, order='F')
        ...
        lcmswrap.applycform(self.address, inmat_p, outmat_p, ctypes.c_size_t(
            np.int(np.size(inmat, 1)/self.no_in_ch)))
        inmat = inmat.reshape(
            (np.int(np.size(inmat, 1)/self.no_in_ch), self.no_in_ch))
        outmat = outmat.reshape(
            (np.int(np.size(outmat, 1)/self.no_out_ch), self.no_out_ch))
        return outmat
...
class gammapform:
    ...
    def __init__(self, filename):
        Lab_grid_LUT_gm_f = np.fromfile(
            binPath, dtype=ctypes.c_float, count=-1, sep='')
        Lab_grid_LUT_gm_f_p = Lab_grid_LUT_gm_f.ctypes.data_as(
            ctypes.POINTER(ctypes.c_float))
        self.address = ctypes.c_void_p(revint.makegammapForm(
            Lab_grid_LUT_gm_f_p, ctypes.c_uint(45)))
    ...
    def apply(self, labin):
        ...
        lab_gm = np.ndarray(shape=(1, np.size(lab, 1)),
                            dtype=ctypes.c_float, order='F')
        lab_p = lab.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        lab_gm_p = lab_gm.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        revint.applygammapForm(self.address, lab_p, lab_gm_p,
                               ctypes.c_size_t(np.int(np.size(lab, 1)/3)))
